...
function deleteBill(obj){
	$.ajax({
		type:"GET",
		url:"ToBilllistServlet",
		data:{method:"modify",billid:obj.attr("billid")},
		dataType:"json",
		success:function(data){
			if(data.delResult == "true"){//删除成功：移除删除行
				cancleBtn();
				obj.parents("tr").find("td").eq(4).html("<span>已取消</span>");
...
$(function(){
	...
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href="ToBilllistServlet?method=delete&billid="+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
	...
	$(".deleteBill").on("click",function(){
		billObj = $(this);
		changeDLGContent("你确定要删除订单【"+billObj.attr("billid")+"】吗？");
		openYesOrNoDLG();
	});
