...
function deleteProvider(obj){
	$.ajax({
		type:"GET",
		url:"DeleteGoodServlet",
		data:{method:"delprovider",goodId:obj.attr("goodId")},
		dataType:"json",
		success:function(data){
			if(data.delResult == "true"){//删除成功：移除删除行
				...
				//alert("对不起，删除供应商【"+obj.attr("goodName")+"】失败");
				changeDLGContent("对不起，删除商品【"+obj.attr("goodName")+"】失败");
			}else if(data.delResult == "notexist"){
				//alert("对不起，供应商【"+obj.attr("goodName")+"】不存在");
				changeDLGContent("对不起，商品【"+obj.attr("goodName")+"】不存在");
			}else{
				//alert("对不起，该供应商【"+obj.attr("goodName")+"】下有【"+data.delResult+"】条订单，不能删除");
				changeDLGContent("对不起，该商品【"+obj.attr("goodName")+"】下有【"+data.delResult+"】，不能删除");
			}
		},
		error:function(data){
			//alert("对不起，删除失败");
...
$(function(){
	...
	$(".deleteProvider").on("click",function(){
		providerObj = $(this);
		changeDLGContent("你确定要删除商品【"+providerObj.attr("goodName")+"】吗？");
		openYesOrNoDLG();
	});
	$(".viewGood").on("click",function(){
		providerObj = $(this);
		window.location.href="ToGoodViewServlet?goodId="+ providerObj.attr("goodId");
